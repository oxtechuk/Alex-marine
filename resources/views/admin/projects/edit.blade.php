...
@section('title', 'تعديل حالة الصيانة — أليكس مارين')
@section('page_title', 'تعديل حالة الصيانة: ' . $project->title_ar)
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div class="d-flex align-items-center gap-3">
        ...
        <div>
            <h4 class="fw-bold text-dark m-0">تعديل حالة الصيانة (Before & After)</h4>
            <p class="text-muted fs-7 m-0">{{ $project->title_ar }}</p>
        </div>
    </div>
    <a href="{{ route('projects.show', $project->slug) }}" target="_blank" class="btn btn-outline-primary rounded-pill px-3 py-2 fs-7 fw-bold">
        <i class="bi bi-box-arrow-up-right me-1"></i> معاينة في الموقع
    </a>
</div>
        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm rounded-4 bg-white p-4 mb-4">
                ...
                <div class="row g-3">
                    <div class="col-md-12">
                        ...
                        <input type="text" name="title_ar" class="form-control form-control-lg rounded-3 fs-6" value="{{ old('title_ar', $project->title_ar) }}" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-bold text-dark fs-7">العنوان بالإنجليزية (اختياري)</label>
                        <input type="text" name="title_en" class="form-control rounded-3" value="{{ old('title_en', $project->title_en) }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="service_id" class="form-select rounded-3">
                            ...
                            @foreach($services as $s)
                                <option value="{{ $s->id }}" {{ old('service_id', $project->service_id) == $s->id ? 'selected' : '' }}>
                                    {{ $s->name_ar }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="location_ar" class="form-control rounded-3" value="{{ old('location_ar', $project->location_ar) }}">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-bold text-dark fs-7">نوع السفينة أو المنشأة (اختياري)</label>
                        <input type="text" name="vessel_type" class="form-control rounded-3" value="{{ old('vessel_type', $project->vessel_type) }}">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-bold text-dark fs-7">مدة التنفيذ (اختياري)</label>
                        <input type="text" name="duration" class="form-control rounded-3" value="{{ old('duration', $project->duration) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 bg-white p-4 mb-4">
                ...
                <div>
                    ...
                    <textarea name="description_ar" class="form-control rounded-3" rows="5">{{ old('description_ar', $project->description_ar ?: $project->short_desc_ar) }}</textarea>
                </div>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm rounded-4 bg-white p-4 mb-4 sticky-top" style="top: 20px;">
                ...
                <div class="form-check form-switch mb-3">
                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ $project->is_active ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="sort_order" class="form-control rounded-3" value="{{ $project->sort_order }}">
                </div>
